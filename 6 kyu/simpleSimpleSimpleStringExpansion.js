...
const stringExpansion = s => {
  return !s ? '' : s.match(/[a-zA-Z]+|[0-9]+(?:\.[0-9]+|)/g)
    .map(el => Number(el) > 10 ? Number(el.split('').reverse()[0]) : Number(el) || el)
    .map((el, i, arr) => isNaN(el) ? el.split('').map(el => arr[i - 1] == 0 ? '' : Array(arr[i - 1]).fill(el)) : '')
    .join()
    .replace(/,/g, '')
}
...
export function getCellAddresses(r: string): string[] {
  let alph: any = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'
  let alphRange: string = alph.slice(alph.indexOf(r.split(':')[0].match(/[A-Z]/g)), alph.indexOf(r.split(':')[1].match(/[A-Z]/g)) + 1)
  let numRange: number = (+r.split(':')[1].replace(/^\D+/g, '')) - (+r.split(':')[0].replace(/^\D+/g, ''))
  return (alphRange.length === 1 && numRange <= 0) || numRange < 0 ? [] : [...Array(numRange + 1)].map((_, i: number) => alphRange.split('').map((el: string) => el + (i + Number(r.split(':')[0].replace(/^\D+/g, ''))))).flat()
}